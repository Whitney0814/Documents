...
let b = 1;
let temp = 0;
let inputNumber = Number(prompt("숫자입력"));
...
const VARIABLECOST = 70;
let noteBook = 170;
let count = 1;
while((noteBook*count) <= FIXEDCOST+VARIABLECOST*count){
    count++;
}
alert(count);
*/
...
// 5. 상근이는 요즘 설탕공장에서 설탕을 배달하고 있다. 상근이는 지금 사탕가게에 설탕을 정확하게 N킬로그램을 배달해야 한다. 설탕공장에서 
